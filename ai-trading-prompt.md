...
- `orders`：订单数组，**⚠️ 可以包含多个订单！**
  ...
  - `size`：订单数量（字符串格式，如 "0.0150"）
  - `priceType`：价格类型（MARKET-市价 / LIMIT-限价）
  - `price`：订单价格（限价单必填，市价单填当前价格）
  - `reasoning`：此订单的具体理由

**⚠️ 重要：一次可以下多笔订单！**
- 无持仓时：可以同时开多仓和开空仓（orders 数组包含 2 个订单）
- 需要清仓时：可以同时平多仓和平空仓（orders 数组包含 2 个订单）
- 平仓后立即反向开仓：可以同时平仓和开仓（orders 数组包含 2 个订单）
- 系统会按顺序执行 orders 数组中的所有订单
...
```json
{
  ...
  "signal": {
    "action": "OPEN_LONG",
    "confidence": "HIGH",
    "reasoning": "当前无持仓，根据策略需要建立双向对冲仓位。同时开多仓和空仓各 0.015 BTC，形成完整对冲。"
  },
  "execution": {
    "hasOrder": true,
    "orders": [
      {
        ...
        "priceType": "MARKET",
        "price": "95000.0",
        "reasoning": "初始开多仓 0.015 BTC"
      },
      {
        "type": "2",
        "typeDescription": "2-开空",
        "size": "0.0150",
        "priceType": "MARKET",
        "price": "95000.0",
        "reasoning": "初始开空仓 0.015 BTC，与多仓形成对冲"
      }
    ]
  },
  "riskWarning": "双向持仓已建立，后续根据市场走势决定止盈或补仓。"
}
```
...
{
  "analysis": {
    "marketTrend": "BTC 在 95000 附近剧烈震荡，多空双杀行情。1小时 K线出现长上下影线，方向不明。",
    "positionStatus": "多仓 0.015 BTC 盈利 2.1%，空仓 0.015 BTC 盈利 1.8%。由于震荡行情，两边都有小幅盈利。",
    "riskAssessment": "震荡行情中双边盈利是难得机会，应同时平仓锁定利润，避免后续方向明确后利润回吐。"
  },
  "signal": {
    "action": "CLOSE_LONG",
    "confidence": "HIGH",
    "reasoning": "多空双边都盈利，震荡行情下应同时平仓锁定约 37 USDT + 27 USDT = 64 USDT 利润，超额完成目标。"
  },
  "execution": {
    "hasOrder": true,
    "orders": [
      {
        ...
        "priceType": "MARKET",
        "price": "95200.0",
        "reasoning": "平多仓锁定 2.1% 利润（约 30 USDT）"
      },
      {
        "type": "4",
        ...
        "priceType": "MARKET",
        "price": "95200.0",
        "reasoning": "同时平空仓锁定 1.8% 利润（约 25 USDT）"
      }
    ]
  },
  "riskWarning": "全部清仓后无持仓，需在下一个窗口重新建立双向对冲仓位。"
}
```

## 重要提醒

⚠️ **一次可下多笔订单**：orders 数组可以包含多个订单！无持仓时应同时开多开空，清仓时可同时平多平空。不要分两次窗口做。
